...
import { brands, getBrandBySlug, getProductsByBrandSlug } from "../../../data/brands";
import { isLocale, locales, type Locale } from "../../../lib/i18n";

type CollectionProfilePageProps = {
  params: Promise<{ locale: string; brand: string }>;
};

function getProfileCopy(locale: Locale, brand: { name: string; slug: string; description?: string; localized?: { [key in Locale]?: { description?: string } } }) {
  if (brand.slug !== "54couture") {
    const description = brand.localized?.[locale]?.description ?? brand.description ?? "";

    return locale === "en"
      ? { back: "Back to Collections", eyebrow: `About ${brand.name}`, title: "Material knowledge shaped by hand.", paragraphs: [description], works: "Selected works", request: "Available on request" }
      : { back: "Zurück zu Collections", eyebrow: `Über ${brand.name}`, title: "Materialwissen, von Hand geformt.", paragraphs: [description], works: "Ausgewählte Arbeiten", request: "Auf Anfrage" };
  }
...
export function generateStaticParams() {
  return locales.flatMap((locale) => brands.filter((brand) => brand.heroImage).map((brand) => ({ locale, brand: brand.slug })));
}

export async function generateMetadata({ params }: CollectionProfilePageProps): Promise<Metadata> {
  ...
  const title = `${brand.name} | Collections`;
  const description = brand.localized?.[locale]?.description ?? brand.description;
  const path = `/${locale}/collections/${brand.slug}`;

  return {
    title,
    description,
    alternates: { canonical: path },
    openGraph: brand.heroImage ? { title, description, url: path, images: [{ url: brand.heroImage, alt: `${brand.name} collection` }] } : undefined,
    twitter: brand.heroImage ? { card: "summary_large_image", title, description, images: [brand.heroImage] } : undefined,
  };
}

export default async function CollectionProfilePage({ params }: CollectionProfilePageProps) {
  const { locale: rawLocale, brand: slug } = await params;
  ...
  const locale = rawLocale as Locale;
  const copy = getProfileCopy(locale, brand);
  const products = getProductsByBrandSlug(brand.slug);
  const basePath = `/${locale}/collections`;
  ...
  return (
    <main className="bg-[#f3f2ef]">
        ...
        <div className="mx-auto max-w-[1540px]">
          ...
          <div className="mt-9 grid gap-10 lg:grid-cols-[1.08fr_0.92fr] lg:items-end">
            <div className="relative aspect-[16/10] overflow-hidden bg-[#181615]">
              <Image alt={`${brand.name} collection`} className="object-cover" fill priority sizes="(min-width: 1024px) 55vw, 100vw" src={brand.heroImage} />
            </div>
            <div className="pb-2">
      ...
      <section className="border-y hairline bg-[#e8eceb] px-5 py-14 lg:px-10 lg:py-20">
        <div className={`mx-auto grid max-w-[1540px] gap-10 ${brand.founderImage ? "lg:grid-cols-[0.27fr_0.3fr_0.43fr]" : "lg:grid-cols-[0.36fr_0.64fr]"}`}>
          <div>
            <p className="text-[0.68rem] uppercase tracking-[0.24em] text-[#667174]">{copy.eyebrow}</p>
