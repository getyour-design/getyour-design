  ...
  return (
    <header className="sticky top-0 z-50 bg-[#f7f6f2]">
      <nav className="flex min-h-[72px] w-full items-center justify-between gap-4 px-5 md:px-10 lg:px-9 2xl:px-[68px]">
        ...
        <div className="hidden max-w-full gap-5 text-[0.725rem] font-semibold uppercase tracking-[0.18em] text-[#10100f] min-[900px]:flex min-[900px]:order-none min-[900px]:border-0 min-[900px]:pt-0 2xl:gap-11 2xl:text-[0.785rem] 2xl:tracking-[0.2em]">
          {navItems.map((item) => {
            const isActive =
        ...
        <div className="hidden shrink-0 items-center justify-end gap-3 text-[0.58rem] font-semibold uppercase leading-none tracking-[0.16em] text-[#10100f] min-[900px]:flex md:gap-4 2xl:gap-5 2xl:text-[0.62rem] 2xl:tracking-[0.18em]">
          ...
          <div className="relative hidden self-stretch items-center sm:flex" ref={languageMenuRef}>
            ...
            >
              ...
              <span aria-hidden="true" className="grid w-3 gap-[3px]">
            ...
            {languageMenuOpen ? (
              <div
                className="absolute right-0 top-[calc(100%+18px)] z-[60] min-w-[112px] border border-black/10 bg-[#f7f6f2] py-2 shadow-[0_14px_30px_rgba(16,16,15,0.12)]"
                id="desktop-language-menu"
              >
                {locales.map((targetLocale) => (
                  <Link
                    aria-current={locale === targetLocale ? "page" : undefined}
                    className={`block px-5 py-2.5 text-left transition hover:bg-black/[0.04] hover:text-black ${
                      locale === targetLocale ? "text-black underline underline-offset-4" : "text-[#667174]"
                    }`}
        ...
        <div className="flex items-center gap-4 text-[0.62rem] font-semibold uppercase tracking-[0.16em] text-[#10100f] min-[900px]:hidden">
      ...
      {mobileMenuOpen ? (
        <div className="absolute inset-x-0 top-full z-50 border-t hairline bg-[#f7f6f2] px-5 pb-7 pt-5 shadow-[0_16px_30px_rgba(16,16,15,0.12)] min-[900px]:hidden">
